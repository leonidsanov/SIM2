...
SEC_CODE				= "SiM2";			-- Код бумаги
interval				= INTERVAL_M30 		-- В данный момент интервал 30 минут
------------------
-- Тело скрипта --
------------------
...
function main()
	while IsRun do
		if os.sysdate().sec == 30 then 			-- Каждые 15 секунд обновляет файл с данными, полученными с сервера (внимание!), поэтому если данные с сервера не пришли файл будет пустой даже если в терминале есть график
			CSV = io.open(getScriptPath().."/SiM2_Data.csv", "w"); -- Создает, или открывает для чтения/добавления файл CSV в той же папке в режиме с возможностью записи, позволяющий изменять существующие файлы и создавать новые файлы (нечитаемые), где находится данный скрипт
